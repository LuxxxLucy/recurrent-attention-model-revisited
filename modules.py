...
class error_network(nn.Module):
    ...
    """
    def __init__(self, input_size, output_size):
        super(error_network, self).__init__()
        self.fc = nn.Linear(input_size, output_size)

    def forward(self, h_t):
        # a_t = F.softmax(self.fc(h_t), dim=1)
        a_t = F.relu( self.fc(h_t))
        # a_t = torch.clamp(a_t,0,1)
        # a_t = torch.sigmoid( self.fc(h_t))
        # a_t = torch.abs( self.fc(h_t))
        return a_t
...
class location_network(nn.Module):
    ...
    def forward(self, h_t):
        ...
        mu = torch.clamp(self.fc(h_t.detach()), -1,1)
